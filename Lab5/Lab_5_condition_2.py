...
def clustering_without_dl(imdir, targetdir, number_clusters):
    ...
    filelist = glob.glob(os.path.join(imdir, "*.png")) + glob.glob(
        os.path.join(imdir, "*.jpg")
    )
    filelist.sort()
    ...
    print(
        f"Кластеризація {len(featurelist)} об'єктів на {number_clusters} кластерів..."
    )

    kmeans = KMeans(
        n_clusters=number_clusters, init="k-means++", n_init=10, random_state=0
